...
for i in range(0,11):
	...
	if iteration_number==0:
		...
		# print(line_number)
		line_to_read3= line3[line_number]
		...
		myfile_lf=open("lf.txt","r")
		line4= myfile_lf.readlines()
		line_to_read4= line4[line_number]
		required_array4= line_to_read4[0:-2]
		temp4= required_array4.split(",")
		lf_bl_code={}
		lf_list=[]
		for i in range(0,len(temp4)):
			temp4[i]= int(temp4[i])
		for i in range(0,len(temp4)-1,2):
			if temp4[i+1]>0:
				lf_bl_code[temp4[i]]=temp4[i+1]
				lf_list.append(temp4[i])
				# print(temp4[i],temp4[i+1])
		...
		if union_val>0:
			...
			print(line_number,chi_sq_normal,union_val)


		lf_only_list= [x for x in lf_list if x not in actual_list]
		actual_only_lf_list= [x for x in actual_list if x not in lf_list]
		lf_atck_int= set(lf_list).intersection(actual_list)



		chi_sq_lf=0

		for i in lf_atck_int:
			observed_value= actual_bl_code[i]
			expected_value= lf_bl_code[i]
			squared_val = (observed_value-expected_value)*(observed_value-expected_value)
			val_to_add= squared_val/expected_value
			chi_sq_lf=chi_sq_lf+val_to_add

		for i in lf_only_list:
			observed_value= 0
			expected_value= lf_bl_code[i]
			squared_val = (observed_value-expected_value)*(observed_value-expected_value)
			val_to_add= squared_val/expected_value
		...
		union_val= len(lf_list)+len(actual_list)-len(lf_atck_int)
		if union_val>0:
			print("################ Link Failure chi sq ################ ")
			print("Window: ",line_number)
			print(line_number,chi_sq_lf,union_val)
